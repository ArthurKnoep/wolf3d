...
** Last update Tue Jan 10 21:49:01 2017 
*/
			 ...
			 sfVector2i mapsize)
{
  ...
  while  ((int)(pos.y + p_act.y + p_inc.y / 20) < mapsize.y &&
  	  (int)(pos.y + p_act.y + p_inc.y / 20) >= 0 &&
	  (int)(pos.x + p_act.x) < mapsize.x &&
  	  (int)(pos.x + p_act.x) >= 0 &&
...
float	raycast(t_client pos, float direction, int **map, sfVector2i mapsize)
{
  float	len_hor;
  float	len_ver;
